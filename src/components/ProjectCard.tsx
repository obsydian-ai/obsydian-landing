    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 h-full">
      ...
      >
        ...
        <motion.div
          ...
          className="relative bg-white rounded-2xl lg:rounded-3xl border border-gray-100/50 p-4 sm:p-6 lg:p-8 h-full transform-gpu overflow-hidden"
        >
          {!isTouchDevice && (
            <motion.div
              className="pointer-events-none absolute -inset-px rounded-2xl lg:rounded-3xl opacity-0 transition duration-300"
              style={{
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div className="space-y-3 sm:space-y-4 lg:space-y-6" style={{ transform: isTouchDevice ? 'none' : "translateZ(40px)" }}>
              <motion.div
              ...
              >
                <div className="flex items-center space-x-2 lg:space-x-3">
                  <motion.div 
                    className={`w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full ${cardTheme.accent}`}
                    animate={{ scale: isHovered && !isTouchDevice ? 1.1 : 1 }}
                    transition={{ duration: 0.4 }}
                  />
                  <h3 className={`text-lg sm:text-xl lg:text-2xl font-bold tracking-tight ${cardTheme.text}`}>{project.name}</h3>
                </div>
                <p className="text-gray-600 text-sm sm:text-base lg:text-lg leading-relaxed">{project.description}</p>
              </motion.div>
            </div>
            ...
            <motion.div
              ...
              className="pt-4 sm:pt-6 lg:pt-8 relative z-20"
              style={{ transform: isTouchDevice ? 'none' : "translateZ(50px)" }}
            >
              <Button 
                variant="ghost"
                className={`group relative px-3 py-1.5 sm:px-4 sm:py-2 lg:px-6 lg:py-3 text-xs sm:text-sm lg:text-base font-medium rounded-full border ${cardTheme.border} transition-all duration-400 md:hover:scale-105 md:hover:shadow-lg ${cardTheme.text} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${theme.primary} active:scale-95`}
                onClick={() => {
              ...
              >
                <span className="relative z-10 flex items-center">
                  ...
                  >
                    <ArrowRight className="ml-1 sm:ml-1.5 lg:ml-2 h-3 w-3 sm:h-3.5 sm:w-3.5 lg:h-4 lg:w-4" aria-hidden="true" />
                  </motion.div>
                </span>
                <motion.div
      ...
      {/* Right Card: Phone Screenshot - Hidden on mobile, visible from lg breakpoint */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
